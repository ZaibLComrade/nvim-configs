...
vim.opt.shiftwidth = 4 -- Auto-indent width is 4 spaces
...
vim.keymap.set('n', '<leader>vt', ':vsplit | terminal<CR>', { noremap = true, silent = true, desc = 'Open vertical split terminal' })

-- Horizontal split terminal
vim.keymap.set('n', '<leader>ht', ':split | terminal<CR>', { noremap = true, silent = true, desc = 'Open horizontal split terminal' })
...
require('lazy').setup({
  ...
  -- Neo tree for folder navigation
  {
  ...
  -- Autosave
  {
    ...
    config = function()
      require('auto-save').setup {
        enabled = true, -- start in autosave mode
        execution_message = {
          message = function()
        ...
        trigger_events = { 'InsertLeave', 'TextChanged' }, -- autosave on leaving insert mode or text change
        condition = function(buf)
          local fn = vim.fn
          local utils = require 'auto-save.utils.data'

          if fn.getbufvar(buf, '&modifiable') == 1 and utils.not_in(fn.getbufvar(buf, '&filetype'), { 'gitcommit', 'markdown' }) then
            return true -- save the file, but we'll clear modified status afterward
          end
          return false -- don't autosave this buffer
        end,
        debounce_delay = 135, -- Minimum time (in ms) between saves
        callback = function()
          -- Reset modified flag after autosave to avoid setting buffer to "modified"
  ...
  { -- Autoformat
    ...
    opts = {
      ...
      format_on_save = function(bufnr)
        ...
        -- NOTE: Disabled autoformating on autosave
        return nil
